...
from agent_plays_balatro.models import (
    GameState,
)  # Card, Joker no longer directly needed here for instantiation
...
class GameStateAgent:
    ...
    def extract_current_game_state(self) -> GameState:  # Added return type hint
        ...
        score_info = self.image_processor.extract_text_from_region(
            capture_data, "score_area"
        )
        deck_info = self.image_processor.extract_text_from_region(
            capture_data, "deck_info_area"
        )
        hands_left = self.image_processor.extract_text_from_region(
            capture_data, "hands_left_area"
        )
        discards_left = self.image_processor.extract_text_from_region(
            capture_data, "discards_left_area"
        )

        player_hand_data = self.card_recognizer.recognize_cards_in_region(
            regions.get("hand_area")
        )
        active_jokers_data = self.joker_recognizer.recognize_jokers_in_region(
            regions.get("joker_area")
        )
